...
{
    return (queue->buffer + (((queue->buffer_start + pos) % (queue->max_msgs))) * queue->msg_size);
}
...
int mqueue_send(mqueue_t *queue, void *msg)
{
    ...
    memcpy(queue_position(queue, queue->buffer_count), msg, queue->msg_size);
    queue->buffer_count++;
...
{
    if (queue == NULL || msg == NULL || queue->buffer == NULL)
    {
        return -1;
    }
    ...
    memcpy(msg, queue_position(queue, 0), queue->msg_size);
    queue->buffer_count--;
    queue->buffer_start++;
